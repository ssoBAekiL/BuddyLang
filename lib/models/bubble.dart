// https://github.com/vi-k/bubble/blob/master/README.md
...
import 'package:buddylang/models/message_bubble.dart';
...
class Bubble extends StatelessWidget {
  ...
  /*  Returns a bubble with a date                 */
  factory Bubble.dateBubble(DateTime date) {
    return Bubble(
  ...
  /*  widgets from the data in our application     */
  /*  Returns a bubble containing the text of the  */
  /*  message and its timestamp                    */
  factory Bubble.fromMessage(Chat chat, int messageIndex, String uid, bool newDay) {
    Message message = chat.messages[messageIndex]; // Message that will be contained in the bubble
    bool isReply = true; // true if the author af the current message doesn't match with the author of the previous message
    bool isOwn; // true if the message has been written by the user using the app
    bool isNewDay = newDay; // true if the date of the current message doesn't match the of the previous message
    ...
    if ((isOwn && isReply) || (isOwn && isNewDay))
      ...
      return Bubble(
          ...
          color: Colors.lightBlue[200],
          child: MessageWidget(message));
    else if ((!isOwn && isReply) || (!isOwn && isNewDay))
